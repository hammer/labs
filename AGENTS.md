...
- Scaling science: compute-optimal training, scaling laws for specific domains (multimodal, forgetting, data mixtures), and **RL post-training scaling** — compute/rollout/step/model-size/environment scaling of RLVR (ScaleRL, ProRL, BroRL, IsoCompute). Tag every such output `rl-scaling` so the family stays queryable
