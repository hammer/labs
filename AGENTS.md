...
- Scaling science: compute-optimal training, scaling laws for specific domains (multimodal, forgetting, data mixtures), and **RL post-training scaling** — compute/rollout/step/model-size/environment scaling of RLVR (ScaleRL, ProRL, BroRL, IsoCompute) **and RL-readiness / stage-aware pipeline work** — methods that optimize an *earlier* stage for downstream RL (coverage/pass@K-preserving SFT like TailSFT, checkpoint-selection-for-RL diagnostics). Tag every such output `rl-scaling` so the family stays queryable
...
6. **Search for new evaluation benchmarks independently** — evals often come from multi-institutional collaborations not tied to a single tracked lab and are easily missed by lab-centric sweeps. Run dedicated searches each sweep cycle: `site:arxiv.org benchmark evaluation LLM 2604`, `site:arxiv.org reasoning evaluation benchmark 2604`, `site:arxiv.org "chain of thought" evaluation benchmark 2604`, `site:arxiv.org long reasoning benchmark 2604`. Pay special attention to benchmarks that stress-test **extended reasoning chains** (long CoT), **reasoning process quality** (step-level correctness), and **test-time compute scaling** — these tend to be important for tracking thinking models (o-series, DeepSeek R1, QwQ) and are often from academic groups rather than labs. Don't limit the sweep to *general* reasoning evals — also search **domain-specific** expert benchmarks in **medicine, law, finance, science, and agentic/tool-use**, which are frequently authored by a tracked lab's academic collaboration and slip through lab-name searches (e.g. **MedXpertQA**, a medical-reasoning eval from PJLab/Tsinghua, ICML 2025, missed for months). Add searches like `site:arxiv.org medical benchmark LLM 2604`, `site:arxiv.org "expert-level" benchmark 2604`, and scan the **ICML/NeurIPS/ICLR Datasets & Benchmarks** accept lists. When a benchmark lists a tracked lab among its affiliations, file it under that lab — and consider filing **both** an `eval` and a companion `paper` output (MedXpertQA is tracked as both).
   - **RL-scaling sweep (dedicated each cycle):** scaling-law work for RL post-training hides behind axis-specific names (prolonged RL, rollout width, budget allocation, verifiable environments) rather than "scaling law". Run: `site:arxiv.org reinforcement learning scaling law LLM <month>`, `site:arxiv.org RL compute sigmoidal <month>`, `site:arxiv.org rollout scaling RLVR <month>`, `site:arxiv.org compute-optimal RL post-training <month>`, and check the related-work sections of newly filed rl-scaling papers for the axis taxonomy (compute/recipe: ScaleRL; steps: ProRL, Open-Reasoner-Zero; rollout width: BroRL; allocation: IsoCompute, Knapsack RL; model size: 2509.25300; environments: RLVE; mechanisms/limits: rlvr-limits, Demystifying; initialization/RL-readiness: TailSFT — stage-aware papers that shape SFT or checkpoint selection around coverage/pass@K so the model RLs better; these hide behind "fine-tuning" titles with no RL keyword, so also run `site:arxiv.org pass@K coverage post-training <month>` and `site:arxiv.org SFT initialization reinforcement learning <month>`). **Tag every accepted output `rl-scaling`** (the canonical facet label; grep `- rl-scaling` for the current set) alongside `rl`.
...
9. **Sweep prose for stale AAII mentions.** After running the score sync (which rewrites structured fields and appends `intelligence_index_history`), run:
   ...
   It extracts every AAII score quoted in descriptions/notes/variant notes/lab blurbs and auto-classifies each against the file's structured scores, its `intelligence_index_history`, named tracked outputs, and AA's live per-variant values. The work-list it prints has four classes: **STALE** (the number equals an old score in the file's own history and carries no era marker — fix it), **CHECK** (an old history score that a live sibling/variant slug also reads — usually a correct mode note, confirm by eye), **UNRESOLVED** (no evidence either way), and **POINT-DIFF** ("N points above X" derived from two scores — re-derive). Baseline 2026-08-04: 127 mentions, 0 unresolved; v4.3 sweep 2026-09-08: 136 mentions, 0 unresolved; 2026-09-21 audit: 185 mentions, 15 STALE after a 28-file rescore, all fixed. The pure pieces live in `scripts/aaii-mentions.ts`; `npm run test:aaii` pins the phrasings and the classification order — run it in every sweep. News titles are exempt by design (historical records). See the full rules in the AAII section below.

   **Still run the old-value grep after a rescore** (next paragraph in the AAII section). The history check closes the coincidental-resolution blind spot for numbers the sync has replaced, but a stale number in a *different* file (a lab blurb, a comparator in another model's description) has no history entry to match, and the K2 Horizon miss of 2026-09-21 ("On the Artificial Analysis Intelligence Index v4.3 it scores 34", four days after the rescore to 31) happened because the sweep skipped this step.
...
model:
  ...
  intelligence_index_version: "AA v4.3.2"   # current AA Intelligence Index version (v4.3 Sep 7 2026; point release v4.3.2 Sep 2026)
```
...
**The version trap.** AA periodically recalibrates the composite. Each version change can drop scores 10+ points as evals are swapped/updated. AA v4.0 (composite of GDPval-AA, τ²-Bench Telecom, Terminal-Bench Hard, SciCode, AA-LCR, AA-Omniscience, IFBench, Humanity's Last Exam, GPQA Diamond, CritPt) shipped late 2025 and silently invalidated many older scores — Granite 4.0 H-Small went 23 → 11 across that migration. **v4.1 (June 2026)** updated GDPval-AA→V2, τ²-Banking→τ³-Banking, and Terminal-Bench Hard→v2.1, recalibrating everything downward again (e.g. Kimi-K2.6 54 → 43, Grok-4.20 49 → 37). **v4.2 (September 4 2026)** was an interim step toward v5: it added AA-Briefcase (private agentic knowledge-work tasks, 15%) and Surge's GDP.pdf (long-PDF document reasoning, 10%), dropped the saturated GPQA Diamond, doubled the held-out share of the index to 40%, and re-anchored the GDPval-AA/Briefcase Elo scales and AA-LCR v1.1 grading. Every one of our 183 tracked scores moved down again, typically 7–10 points (GPT-5.6 Sol 61 → 51, GLM-5.3 60 → 49, Grok 4.6 61 → 51, K2 Horizon 47 → 38). **v4.3 (current, September 7 2026; [changelog](https://artificialanalysis.ai/articles/artificial-analysis-intelligence-index-v4-3))** swapped Terminal-Bench 2.1 → **Terminal-Bench 4.0** (66 tasks × 3 repeats) and τ³-Banking → **AutomationBench-AA** (Zapier collaboration; private 657-task split across six business domains, partial credit per completed objective, zero for any guardrail violation), lifting the private share to 45%. Weights: AA-Briefcase 15, GDPval-AA v2 10, AutomationBench-AA 5, Terminal-Bench 4.0 10, SciCode 10, AA-Omniscience 15 (accuracy 10 + non-hallucination 5), GDP.pdf 10, AA-LCR v1.1 5, HLE 10, CritPt 10. The shift was two-directional this time: the top fell 2–8 points (Claude Fable 5.1 57 → 53, GPT-6 Astra 55 → 53, Claude Sonnet 5 45 → 38, Gemini 3.5 Flash 42 → 33) while the **floor rose** — models that read 1–4 on v4.2 now read 5–9 (GPT-4 1 → 7, Mixtral 8x7B 1 → 5; the leaderboard floor is 4.83) — so "scores 1 on AA" prose and "well above the median" claims both went stale, not just the headline numbers. All 188 tracked score blocks (186 files) carried v4.3 as of 2026-09-08, 187 of them with a v4.2 history entry (MiniCPM5-2B was filed at v4.3 on Sep 8). **Point releases v4.3.1 and v4.3.2 (September 2026; only the [methodology page's version history](https://artificialanalysis.ai/methodology/intelligence-benchmarking) records them, there is no article)** changed graders, not evals: v4.3.1 refreshed the AA-Briefcase and GDPval-AA pairwise judge panels to Claude Opus 5, GPT-5.6 Sol, and Gemini 3.8 Flash (rubric panels unchanged); v4.3.2 moved GDPval-AA to v2.1, with the Elo scale anchored to DeepSeek V4.1 Flash (max) at 1600 and ratings fitted with a Crowd-BT model, and refitted each AA-Briefcase v1.1 pairwise scope with Crowd-BT (anchor unchanged at GPT-5.5 medium = 1000). The 2026-09-23 sync re-tagged all **198 score blocks (196 files) to "AA v4.3.2"** with **no rounded score moving** and no history appended (same major.minor, same score); note that the leaderboard RSC payload still labels itself "v4.3" while the methodology and model pages say v4.3.2, so read the version from the methodology page, not the payload. Treat any score tagged below the current version as needing re-verification; refresh the whole set with `npm run fetch-aa-intelligence` (bump its `AAII_VERSION` constant, and `CURRENT_VERSION` in `scripts/scan-aaii-mentions.ts`, first).

**Score history.** Every model block carries `intelligence_index_history` — superseded readings, newest first, each `{score, version, until}` where `until` is the date we observed the replacement. `fetch-aa-intelligence` appends automatically whenever a synced score changes value, **and on every recalibration (a change in the index's major.minor version, e.g. v4.1.1 → v4.2 or v4.2 → v4.3) even when the rounded number happens to coincide** — the old reading was measured on a different eval mix, so it is a distinct observation worth keeping. A point release (v4.1 → v4.1.1) with the same score only re-tags. Grouped files with more than one `intelligence_index` block (`zai/glm-5.yaml`, `kuaishou/kat-coder.yaml`) are skipped by the script and must be updated by hand, unit by unit, with the same history convention. Never edit the trail by hand except to remove entries that were our own anchoring bugs rather than AA rescores (judgment call — the Ling-1T "10" stayed because it *is* what we displayed). Model pages show the most recent superseded reading ("was N") with the full trail in the hover title. Backfilled from git history 2026-08-03 (93 files, 119 entries).
...
**The scanner checks the file's own history first, but after any rescore ALSO run an independent old-value grep.** Since 2026-09-21 `scan-aaii-mentions` classifies a number that equals an old `intelligence_index_history` score as STALE (or CHECK when a live sibling reads it) *before* it consults the live leaderboard, so the coincidental-resolution blind spot no longer hides in-file staleness; a marker like "38 on v4.2" only counts for the number it sits next to (the window is cut at the neighbouring score, so "it scores 34 (38 on v4.2; …)" cannot launder the 34). What it still cannot see is a stale comparator quoted in another file, and AA also re-runs evals *within* a version (28 scores moved between 2026-09-17 and 09-21), so a sweep starts with the sync dry-run, not the scanner. The v4.1.1 sweep (2026-08-09) had ~a dozen such misses caught only by hand: Opus 5 "61"→63 (61 = current gpt-5-6-sol), qwen3.6-max "40"→41 (40 = qwen3-6-plus), Grok 4.5 "54"→56, GLM-5.2 "51"→53, Kimi K3 "57"→60 — plus sub-variant/comparator figures the scanner can't map to a slug at all (GPT-5.4-Mini "40"→41, gemma-4 E2B "9"→10, Nova Micro "5"→4). So after the sync, for every *rescored* file, grep its prose for the OLD score in an AA context, treating every hit as work-list alongside the scanner's UNRESOLVED set:
...
**Mode-sibling fill (same checkpoint, done by the script).** AA often files the openness record under one mode name (`glm-4-6`) while we anchor the other (`glm-4-6-reasoning`). Reasoning/non-reasoning/effort variants of one checkpoint are the same weights under the same license, so `fetch-aa-openness` groups AAOI records by checkpoint root (trailing mode tokens stripped — the `--audit-modes` logic) and, when the anchor has no record but its siblings agree, writes the sibling value tagged `"AA Openness Index v1.0 (mode sibling: <slug>)"`. A direct record for the anchor later overwrites the tag. The same pass **audits** anchors whose mode siblings carry a *different* AAOI and prints them as disagreements — that means AA treated them as distinct checkpoints or one slug is mis-rooted; resolve by hand, never by averaging.
...
**Backfill workflow.** Run `npm run fetch-aa-openness` (or `npm run fetch-aa-openness -- --dry-run`). The script pulls AA's openness leaderboard via its React Server Component endpoint (314 records as of 2026-08), matches each labindex output's AA URL slug, and writes the score+version into the YAML. It also lists slugs that have an AA URL but no AAOI entry — those are the manual-attention cases. **Payload-shape trap (2026-08, again 2026-09):** the bulk openness records carry only `id` (uuid) + full variant `name` + `opennessIndex` — no `slug`. In Aug 2026 the script joined on the uuid against the Intelligence leaderboard payload; in Sep 2026 that payload dropped model ids entirely (its records now open with `{"slug":…,"shortName":…}`), so the join became **name-based**: the Intelligence page's secondary `{"slug","name"}` component carries the same full variant name ("DeepSeek V4 Pro 0813 (Reasoning, Max Effort)") and maps it to the slug. The script refuses to write if fewer than 300 name→slug pairs or 200 openness records parse. "Parsed 0 model→AAOI pairs", "Only N pairs parsed", or a sync that suddenly sees 1 record means a sentinel drifted again; fix the parser (all three AA scripts share the trap), never hand-enter AAOI values.
